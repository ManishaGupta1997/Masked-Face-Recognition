...
def stream(pb_path, node_dict,ref_dir,camera_source=0,resolution="480",to_write=False,save_dir=None):
    ...
    frame_count = 0
    FPS = "loading"
    face_mask_model_path = r"C:\Users\manis\Desktop\Capstone\Deep Learning masked face detection, recognition\17. Real time face detection, facial mask detection, and face recognition\Realtime_face_recognition\face_mask_detection.pb"
    margin = 40
        ...
        if ret is True:
            ...
            if len(bboxes) > 0:
                for num, bbox in enumerate(bboxes):
                    ...
                    if class_id == 0:
                        color = (255,255,255)  # (B,G,R) --> Green(with masks)
                    else:
                    ...
                    cv2.putText(img, "{},{}".format(id2class[class_id], name), (bbox[0] + 30, bbox[1] - 30),
                                cv2.FONT_HERSHEY_PLAIN, 2, color, 2, 2)

                    if name != "":

                        while (name not in present_list):
                            present_list.append(name)
                            timestamp_list.append(datetime.now().strftime("%I:%M:%S_%p"))
            ...
            #----image display
            cv2.imshow("Capstone project", img)
...
if __name__ == "__main__":
    ...
    pb_path = r"C:\Users\manis\Desktop\Capstone\Deep Learning masked face detection, recognition\16. Training skills episode 2\2.3 pb_model_select_num=15.pb"
    ...
    ref_dir = r"C:\Users\manis\Desktop\Capstone\Deep Learning masked face detection, recognition\17. Real time face detection, facial mask detection, and face recognition\dataset"
    ref_dir = r"C:\Users\manis\Desktop\Capstone\Deep Learning masked face detection, recognition\17. Real time face detection, facial mask detection, and face recognition\test_xxx"
...
x= pd.Series(present_list)
y=pd.Series(timestamp_list)

data={'Name':present_list,'Time':timestamp_list}
...
file_name = f'{datetime.now().strftime("%Y-%m-%d")}.xlsx'
